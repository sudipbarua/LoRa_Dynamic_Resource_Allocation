...
def placeRandomlyInRange(number, nrIntNodes, locArray, xRange, yRange, refLoc, bestValue,
    ...
    """ Place nodes randomly in a range, with a specified proportion of nodes assigned to each region.
    Parameters
    ----------
    number : int
    ...
    locArray: array
        Array to store node locations and parameters.
    xRange: [xmin, xmax]
        Range for x-coordinate placement.
    yRange: [ymin, ymax]
        Range for y-coordinate placement.
    refLoc: array
        Reference locations (e.g., base stations).
    bestValue: list
        [bestDist, bestSF, bestBW] for node to BS.
    radius: float
    ...
    distMatrix: array
        Distance thresholds for each region, separated by SF and BW.
    Returns
    -------
    locArray: array
        Updated array with node locations and parameters.
    """
    temp = 0
